...
async def single_query(request: SingleQueryRequest, current_user: dict = Depends(get_current_user)):
    """Query a single question with optional threshold"""
    if not chatbot:
        raise HTTPException(status_code=503, detail="Chatbot not initialized")
    
    logger.info(f"User {current_user['username']} submitted single query: '{request.question[:50]}...'")
    
    try:
        result = chatbot.query(request.question, threshold=request.threshold)
        logger.info(f"Single query processing completed successfully")
